...
export class DateRange {
  constructor(start, end, options = {}) {
    ...
    this.start = s || s === 0 ? this.moment(s) : this.moment(-8640000000000000);
    this.end = e || e === 0 ? this.moment(e) : this.moment(8640000000000000);
  }
  ...
  add(other, options = { adjacent: false }) {
    if (this.overlaps(other, options)) {
      return new this.constructor(this.moment.min(this.start, other.start), this.moment.max(this.end, other.end));
    }
  ...
  snapTo(interval) {
    ...
    if (!r.start.isSame(this.moment(-8640000000000000))) {
      r.start = r.start.startOf(interval);
    }
    if (!r.end.isSame(this.moment(8640000000000000))) {
...
export function extendMoment(moment) {
  ...
  moment.range = function range(start, end) {
    ...
    if (typeof start === 'string' && INTERVALS.hasOwnProperty(start)) {
      return new DateRange(moment(m).startOf(start), moment(m).endOf(start), { moment });
    }

    return new DateRange(start, end, { moment });
  };
  ...
  moment.rangeFromInterval = function(interval, count = 1, date = moment()) {
    ...
    return new DateRange(dates, null, { moment });
  };
